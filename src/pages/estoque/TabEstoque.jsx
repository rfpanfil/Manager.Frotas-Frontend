...
export default function TabEstoque() {
    ...
    // --- ESTADOS DE DADOS ---
    const { data: itens = [] } = useQuery({ queryKey: ['itensEstoquePrincipal'], queryFn: async () => (await api.get('/estoque/itens')).data });
    const { data: categoriasBD = [] } = useQuery({ queryKey: ['categoriasEstoque'], queryFn: async () => (await api.get('/estoque/categorias').catch(() => ({ data: [] }))).data });
    const { data: unidadesBD = [] } = useQuery({ queryKey: ['unidadesBD'], queryFn: async () => (await api.get('/estoque/unidades').catch(() => ({ data: [] }))).data });
    const { data: bases = [] } = useQuery({ queryKey: ['bases'], queryFn: async () => (await api.get('/bases/').catch(() => ({ data: [] }))).data });
    const { data: usuarios = [] } = useQuery({ queryKey: ['usuarios'], queryFn: async () => (await api.get('/usuarios/').catch(() => ({ data: [] }))).data });
    const { data: codigosData } = useQuery({ queryKey: ['seriaisPatrimonios'], queryFn: async () => (await api.get('/estoque/seriais-patrimonios').catch(() => ({ data: { seriais: [], patrimonios: [] } }))).data });

    const seriaisUsados = codigosData?.seriais || [];
    const patrimoniosUsados = codigosData?.patrimonios || [];
    ...
    async function handleSalvarCategoria(e) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['categoriasEstoque'] }); queryClient.invalidateQueries({ queryKey: ['unidadesBD'] }); queryClient.invalidateQueries({ queryKey: ['bases'] }); queryClient.invalidateQueries({ queryKey: ['usuarios'] }); queryClient.invalidateQueries({ queryKey: ['seriaisPatrimonios'] });
    ...
    function handleNovaUnidade() {
        ...
        if (!unidadesBD.find(u => u.nome === nomeUpper)) {
            queryClient.invalidateQueries({ queryKey: ['unidadesBD'] });
        }
    ...
    async function handleSubmitMolde(e) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['itensEstoquePrincipal'] });
        } catch (error) {
    ...
    async function handleMovimento(e) {
        ...
        try {
            // Sanitiza o payload: converte strings vazias em null para o Pydantic não rejeitar
            const payload = { ...formMov, tipo_movimento: tipoMov };

...
export default function DashboardGastos() {
    ...
    async function exportarPDF() {
        ...
        try {
            ...
            // --- NOVA TABELA: DETALHAMENTO DE TIPOS DE GASTO (Cobre as fatias "Outros") ---
        ...
        <div>
            ...
            {renderPDFCharts && (
                <div style={{ position: 'fixed', left: -10000, top: 0 }}>
                    ...
                    <div ref={pdfPizzaRef} style={{ width: PDF_W, height: PDF_H, background: '#1a202c', padding: 20, display: 'flex', alignItems: 'center' }}>
                        ...
                        {/* A - Gráfico (Agora com MUITO mais espaço para a legenda lateral não bater) */}
                        <div style={{ width: 550, height: PDF_H }}>
                            <PieChart width={550} height={PDF_H}>
                                <Pie
                                    data={pizzaLimpa}
                                    cx={275} // Centraliza no novo container de 550px
                                    cy={PDF_H / 2}
                                    outerRadius={100} 
                                    minAngle={0}
                                    dataKey="value" 
                                    nameKey="name"  
                                    label={renderPizzaLabel}
                                    labelLine={{ stroke: '#a0aec0', strokeWidth: 1, length1: 15, length2: 25 }}
                                    isAnimationActive={false}
                                >
                                    {pizzaLimpa.map((entry, index) => (
                                        <Cell key={`pdf-cell-${index}`} fill={CORES_PIZZA[index % CORES_PIZZA.length]} />
                                    ))}
                                </Pie>
                            </PieChart>
                        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div className="chart-section" ref={chartPizzaRef}>
                    ...
                    {/* Alterado para flexbox para colocar a lista ao lado do gráfico */}
                            ...
                            <ResponsiveContainer>
                                <PieChart>
                                    <Pie 
                                        ...
                                        minAngle={0} 
                                        label={renderPizzaLabel}
                                        labelLine={{ stroke: '#a0aec0', strokeWidth: 1, length1: 15, length2: 20 }} 
                                    >
                                        {pizzaLimpa.map((entry, index) => (
                                    ...
                                    <Tooltip 
                                        formatter={(value) => `R$ ${Number(value).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
                                        cursor={{ fill: 'transparent' }} 
                                        contentStyle={{ backgroundColor: '#1a1e29', border: '1px solid #4a5568', borderRadius: '5px' }} 

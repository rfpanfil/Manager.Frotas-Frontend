...
export default function TabPneus() {
    ...
    const { can } = useAuth();
    const { data: pneus = [] } = useQuery({ queryKey: ['pneus'], queryFn: async () => (await api.get('/pneus/')).data });
    const { data: veiculos = [] } = useQuery({ queryKey: ['veiculosResumo'], queryFn: async () => (await api.get('/veiculos/resumo')).data });
    const { data: resumo = { estoque: 0, em_uso: 0, manutencao: 0, sucata: 0, cpk_medio: 0 } } = useQuery({ queryKey: ['pneusResumo'], queryFn: async () => (await api.get('/pneus/dashboard-resumo')).data });
    ...
    const { data: medidas = [] } = useQuery({ queryKey: ['pneusMedidas'], queryFn: async () => (await api.get('/pneus/medidas')).data });
    ...
    async function handleSalvarMedida(e) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['pneus'] }); queryClient.invalidateQueries({ queryKey: ['veiculosResumo'] }); queryClient.invalidateQueries({ queryKey: ['pneusResumo'] }); queryClient.invalidateQueries({ queryKey: ['pneusMedidas'] });

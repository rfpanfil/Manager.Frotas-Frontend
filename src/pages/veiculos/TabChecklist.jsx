...
export default function ChecklistMensal() {
    ...
    // ESTADOS DE DADOS (Movidos para evitar Lexical Declaration)
    const { data: veiculosStatus = [] } = useQuery({ queryKey: ['checklistsDashboard', filtroData], queryFn: async () => { const [ano, mes] = filtroData.split('-'); return (await api.get(`/checklists/dashboard`, { params: { mes, ano } })).data; }, staleTime: 300000 });
    const { data: veiculosDetalhados = [] } = useQuery({ queryKey: ['veiculosResumo'], queryFn: async () => (await api.get('/veiculos/resumo')).data, staleTime: 300000 });
    const { data: checklistItensDef = [] } = useQuery({ queryKey: ['checklistsDefinicoes'], queryFn: async () => (await api.get('/checklists/definicoes')).data, staleTime: 300000 });
    const { data: usuarios = [] } = useQuery({ queryKey: ['usuarios'], queryFn: async () => (await api.get('/usuarios/')).data, staleTime: 300000 });
    ...
    // Removido useEffect que sabotava o cache
    ...
    async function handleSubmitChecklist(e, statusFinal) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['checklistsDashboard'] }); queryClient.invalidateQueries({ queryKey: ['veiculosResumo'] }); queryClient.invalidateQueries({ queryKey: ['checklistsDefinicoes'] });
    ...
    async function handleExcluirChecklist(id) {
        ...
        try { await api.delete(`/checklists/${id}`); toast.success("Excluído!"); queryClient.invalidateQueries({ queryKey: ['checklistsDashboard'] }); queryClient.invalidateQueries({ queryKey: ['veiculosResumo'] }); queryClient.invalidateQueries({ queryKey: ['checklistsDefinicoes'] }); }
    ...
    async function handleSaveItem(e) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['checklistsDefinicoes'] });
        } catch (error) { toast.error("Erro ao salvar item"); }
    }

    async function handleDeleteItem(id) {
        if (!confirm("Remover este item?")) return;
        try { await api.delete(`/checklists/definicoes/${id}`); queryClient.invalidateQueries({ queryKey: ['checklistsDefinicoes'] }); }

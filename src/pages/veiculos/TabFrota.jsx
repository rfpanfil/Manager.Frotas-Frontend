// Arquivo: frontend/src/pages/veiculos/TabFrota.jsx
...
export default function Veiculos() {
    ...
    // ESTADOS PARA AS LISTAS DO BANCO
    ...
    async function carregarDados() {
        try {
            const [resV, resB, resT, resM] = await Promise.all([
                api.get('/veiculos/'),
                api.get('/bases/'),
                api.get('/opcoes/tipos-veiculo'),
                api.get('/opcoes/marcas-veiculo')
            ]);
            setVeiculos(resV.data);
            setBases(resB.data);
            setTiposVeiculo(resT.data);
            setMarcasVeiculo(resM.data);
                    ...
                    <tbody>
                        {veiculosFiltrados.map((v) => (
                            <tr key={v.id}>
                                ...
                                <td>{v.seguradora || '-'}</td>
                                <td>{v.modelo_pneu || '-'}</td>
                                {!isReadOnly && <td style={{ textAlign: 'center' }}><button onClick={() => abrirModal(v)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3182ce', marginRight: '10px' }}><Edit size={18} /></button><button onClick={() => handleDelete(v.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#e53e3e' }}><Trash2 size={18} /></button></td>}
                            </tr>
                        ))}
            ...
            {modalAberto && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
                    <div className="form-card" style={{ position: 'relative', width: '1100px', maxHeight: '90vh', overflowY: 'auto', background: '#1a202c', border: '1px solid #4a5568', padding: '30px' }}>
                        ...
                        <h3 style={{ marginTop: 0, marginBottom: '20px', color: '#00d68f' }}>{editandoId ? 'Editar Veículo' : 'Novo Veículo'}</h3>

                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>

                            <div style={{ background: '#2d3748', padding: '10px', borderRadius: '5px', textAlign: 'center', border: '1px dashed #00d68f' }}>
                                <label style={{ color: '#00d68f', fontSize: '0.8rem', textTransform: 'uppercase' }}>Identificação Automática</label>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
                                ...
                                <div className="input-group">
                                    ...
                                    <select name="tipo" value={form.tipo} onChange={(e) => handleSelectChange(e, '/opcoes/tipos-veiculo', setTiposVeiculo, 'tipo')}>
                                        ...
                                        <option value="ADD_NEW" style={{ fontWeight: 'bold', color: '#00d68f' }}>+ Cadastrar Novo Tipo...</option>
                                    </select>
                                </div>
                                ...
                                <div className="input-group">
                                    ...
                                    <select name="marca" value={form.marca} onChange={(e) => handleSelectChange(e, '/opcoes/marcas-veiculo', setMarcasVeiculo, 'marca')}>
                                        ...
                                        <option value="ADD_NEW" style={{ fontWeight: 'bold', color: '#00d68f' }}>+ Cadastrar Nova Marca...</option>
                                    </select>
                                </div>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
                                ...
                                <button type="submit" className="btn-add" style={{ backgroundColor: '#00d68f', color: '#000' }}>Salvar Veículo</button>
                            </div>
                        </form>
                    </div>
                </div>

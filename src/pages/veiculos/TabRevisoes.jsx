...
const customSelectStyles = {
    ...
    control: (base, state) => ({ ...base, backgroundColor: '#2d3748', borderColor: '#444', color: 'white', minHeight: '38px', boxShadow: state.isFocused ? '0 0 0 1px #8B5CF6' : 'none', '&:hover': { borderColor: '#8B5CF6' } }),
...
export default function Revisoes() {
    ...
    const { data: revisoes = [] } = useQuery({ queryKey: ['revisoes'], queryFn: async () => (await api.get('/revisoes/')).data });
    const { data: veiculos = [] } = useQuery({ queryKey: ['veiculosResumo'], queryFn: async () => (await api.get('/veiculos/resumo')).data });
    ...
    async function criarPlano(e) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['revisoes'] }); queryClient.invalidateQueries({ queryKey: ['veiculosResumo'] });
    ...
    async function handleDelete(id) {
        if (confirm("Excluir este plano de revisão completamente? (O histórico também será apagado)")) {
            try { await api.delete(`/revisoes/${id}`); queryClient.invalidateQueries({ queryKey: ['revisoes'] }); queryClient.invalidateQueries({ queryKey: ['veiculosResumo'] }); }
                        ...
                        {revisoesFiltradas.map((r) => {
                            ...
                            return (
                                <tr key={r.id}>
                                    ...
                                    <td><strong style={{ color: saldo < 0 ? '#e53e3e' : '#8B5CF6' }}>{saldo} km</strong></td>
                                    <td>
                                        {r.status === 'Em Dia' && <span style={{ color: '#8B5CF6' }}><CheckCircle size={14} style={{ display: 'inline' }} /> Em Dia</span>}
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            {can('revisoes.criar') && (
                                                <button onClick={() => { setRevisaoSelecionada(r); setFormRegistro({ ...formRegistro, km_revisao: kmAtual, observacao: '' }); setModalRegistroAberto(true); }} className="btn-add" style={{ padding: '5px 10px', background: '#8B5CF6', color: '#fff', fontSize: '0.8rem' }}>
            ...
            {modalRegistroAberto && revisaoSelecionada && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div className="form-card" style={{ width: '500px', background: '#1a202c', border: '1px solid #4a5568', padding: '30px', position: 'relative' }}>
                        ...
                        <h3 style={{ marginTop: 0, color: '#8B5CF6' }}><Wrench size={20} style={{ display: 'inline', marginRight: 8 }} />Registrar Manutenção Realizada</h3>
                        ...
                        <form onSubmit={registrarExecucao} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            ...
                            <button type="submit" className="btn-add" style={{ backgroundColor: '#8B5CF6', color: '#fff', padding: 15, fontSize: '1rem', marginTop: 10 }}>
                                Confirmar Execução
                            </button>
                        </form>
                    </div>
                                ...
                                <ul style={{ listStyle: 'none', padding: 0, margin: 0, borderLeft: '2px solid #4a5568', marginLeft: 10 }}>
                                    {historicoLogs.map(h => (
                                        <li key={h.id} style={{ position: 'relative', paddingLeft: 20, paddingBottom: 25 }}>
                                            <div style={{ position: 'absolute', left: -7, top: 0, width: 12, height: 12, borderRadius: '50%', background: '#8B5CF6' }}></div>

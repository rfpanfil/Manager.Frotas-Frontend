// Arquivo: frontend/src/pages/DashboardVeiculos.jsx
...
export default function DashboardVeiculos() {
    ...
    useEffect(() => {
        carregarDashboard();
    }, [periodo, filtros, usarFiltroPeriodo]);
    ...
    async function carregarDashboard() {
        ...
        try {
            ...
            setDados(response.data); // Agora 'dados' é uma lista (Array) de veículos
        } catch (error) { console.error("Erro dashboard veiculos", error); }
    }
    ...
    if (!dados) return <div style={{ padding: '30px', color: 'white' }}>Carregando dados da frota...</div>;

    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h1><Truck style={{ marginRight: '10px' }} /> Dashboard Frota (Analítico)</h1>
            ...
            <div className="filter-bar">
                ...
                <div className="filter-item" style={{ position: 'relative' }}>
                    ...
                    <button onClick={() => setMenuPeriodoAberto(!menuPeriodoAberto)} style={{ background: usarFiltroPeriodo ? '#1a202c' : '#151821', color: 'white', border: usarFiltroPeriodo ? '1px solid #00d68f' : '1px solid #444', padding: '8px', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, minWidth: '180px', justifyContent: 'space-between' }}>
                            ...
                            dados.map((linha, index) => (
                                <React.Fragment key={index}>
                                    <tr style={{ borderBottom: '1px solid #2d3748' }}>
                                        ...
                                        <td style={{ padding: '15px', textAlign: 'center' }}>
                                            <button
                                                ...
                                                style={{
                                                    ...
                                                    border: 'none',
                                                    color: '#00d68f', // Cor padrão do sistema (verde loop)
                                                    cursor: 'pointer',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                        ...
                                        {/* TOTAL GERAL */}
                                        <td style={{ textAlign: 'right', fontWeight: 'bold', color: '#00d68f' }}>
                                        ...
                                        <td style={{ textAlign: 'center' }}>
                                            <span style={{
                                                ...
                                                background: ((modoPR === 'acumulado' ? linha.pr_acumulado : linha.pr_evento_medio) || 0) > 2 ? 'rgba(229, 62, 62, 0.2)' : 'rgba(0, 214, 143, 0.1)',
                                                color: ((modoPR === 'acumulado' ? linha.pr_acumulado : linha.pr_evento_medio) || 0) > 2 ? '#e53e3e' : '#00d68f',
                                                padding: '4px 8px',
                                                borderRadius: '4px',
                                                fontWeight: 'bold',
                                                fontSize: '0.85rem'
                                    ...
                                    {linhasExpandidas[index] && (
                                        <tr style={{ background: '#1a202c' }}>
                                            <td colSpan="8" style={{ padding: '15px' }}>
                                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '10px', paddingLeft: '40px', borderLeft: '4px solid #00d68f' }}>
                                                    ...
                                                    {Object.entries(linha.detalhamento).map(([tipo, valor]) => (
                                                        <div key={tipo} style={{ background: '#2d3748', padding: '8px', borderRadius: '4px', border: '1px solid #4a5568' }}>
                                                            ...
                                                            <div style={{ color: '#00d68f', fontWeight: 'bold' }}>

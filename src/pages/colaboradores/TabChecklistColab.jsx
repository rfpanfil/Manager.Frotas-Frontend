...
export default function TabChecklistColab() {
    ...
    const { data: colaboradoresStatus = [] } = useQuery({ queryKey: ['checklistsColabDashboard', filtroData], queryFn: async () => { const [ano, mes] = filtroData.split('-'); return (await api.get(`/checklists-colab/dashboard`, { params: { mes, ano } })).data; }, staleTime: 1000 * 60 * 5 });
    const { data: checklistItensDef = [] } = useQuery({ queryKey: ['checklistsColabDefinicoes'], queryFn: async () => (await api.get('/checklists-colab/definicoes')).data, staleTime: 1000 * 60 * 5 });
    const { data: configCargos = [] } = useQuery({ queryKey: ['checklistsColabConfig'], queryFn: async () => (await api.get('/checklists-colab/config')).data, staleTime: 1000 * 60 * 5 });
    const { data: cargosDisponiveis = [] } = useQuery({ queryKey: ['cargos'], queryFn: async () => (await api.get('/usuarios/cargos/lista')).data, staleTime: 1000 * 60 * 5 });
    ...
    async function carregarTudo() {
        queryClient.invalidateQueries({ queryKey: ['checklistsColabDashboard'] });
        queryClient.invalidateQueries({ queryKey: ['checklistsColabDefinicoes'] });
        queryClient.invalidateQueries({ queryKey: ['checklistsColabConfig'] });
    ...
    async function handleSaveItem(e) {
        ...
        try {
            ...
            const res = await api.get('/checklists-colab/definicoes'); queryClient.invalidateQueries({ queryKey: ['checklistsColabDefinicoes'] });
        } catch (e) { toast.error("Erro"); }
    }
    async function handleDeleteItem(id) {
        if (confirm("Excluir item?")) { await api.delete(`/checklists-colab/definicoes/${id}`); queryClient.invalidateQueries({ queryKey: ['checklistsColabDefinicoes'] }); }
    }
        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: 20 }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' , flexWrap: "wrap"}}>
                    ...
                    {can('checklists.baixar') && ( <button onClick={() => { const doc = new jsPDF(); doc.text(`Resumo - ${filtroData}`, 14, 10); autoTable(doc, { head: [["Colaborador", "Cargo", "Status", "Data"]], body: colabFiltrados.map(c => [c.nome, c.tipo_colaborador, c.status_checklist, c.data_checklist ? new Date(c.data_checklist).toLocaleDateString() : '-']), startY: 20 }); doc.save("resumo.pdf"); }} className="btn-secondary" style={{ background: '#4a5568', color: 'white', padding: '10px 15px', border: 'none', borderRadius: 5, cursor: 'pointer' }}>PDF Resumido</button> )}
                    {can('checklists.baixar') && ( <button onClick={() => gerarRelatorioDetalhadoColab(colabFiltrados)} className="btn-add" style={{ background: '#e53e3e', color: 'white', padding: '10px 15px' }}><FileText size={18} style={{ marginRight: 5 }} /> Completo</button> )}
                </div>
                ...
                <table style={{ width: '100%' }}>
                    ...
                    <tbody>
                        {colabFiltrados.length === 0 ? (
                            ...
                            <tr key={c.id}>
                                <td><strong style={{ color: '#8B5CF6' }}>{c.nome}</strong><br /><small style={{ color: '#a0aec0' }}>{c.cpf}</small></td>
                                <td>{c.tipo_colaborador}</td>
                                <td>
                                    {c.status_checklist === 'APROVADO' && <span style={{ color: '#8B5CF6' }}><CheckCircle size={14} /> Aprovado</span>}
                                ...
                                <td style={{ textAlign: 'right' }}>
                                    {c.checklist_id && can('checklists.baixar') && ( <button onClick={() => gerarRelatorioDetalhadoColab([c])} style={{ background: '#3182ce', color: 'white', border: 'none', padding: '6px', borderRadius: 4, cursor: 'pointer', marginRight: 5 }}><FileText size={16} /></button> )}
            ...
            {/* MODAL CHECKLIST */}
            {showModalChecklist && (
                <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, overflowY: 'auto', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '20px' , flexWrap: "wrap"}}>
                    <div className="modal-content-responsivo px-2 py-4 md:px-10 md:py-6" style={{ background: '#1a202c', width: '100%', maxWidth: '900px', borderRadius: 8, height: 'fit-content', margin: 'auto' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: 15, background: '#2d3748', padding: 15, borderRadius: 5 }}>
                        ...
                        <div className="checklist-scroll-area" style={{ maxHeight: '65vh', overflowY: 'auto', overflowX: 'hidden', boxSizing: 'border-box', paddingBottom: '15px' }}>
                            {Object.keys(itensPorCategoria).map(cat => (
                                <div key={cat} style={{ marginBottom: 20 }}>
                                    <h3 style={{ borderBottom: '1px solid #444', color: '#8B5CF6', paddingBottom: 5 }}>{cat}</h3>
                                    {itensPorCategoria[cat].map(def => {
                                        ...
                                        return (
                                            <div key={chave} className="checklist-item-grid" style={{ border: '1px solid #4a5568', borderRadius: '8px', padding: '12px', marginBottom: '15px', boxSizing: 'border-box', width: '100%' }}>
                                                {/* ÁREA NOME */}
                                                <div className="chk-area-name" style={{ display: 'flex', alignItems: 'center', gap: '10px' , flexWrap: "wrap"}}>
                                                ...
                                                {/* ÁREA FOTO */}
                                                <div className="chk-area-foto" style={{ display: 'flex', alignItems: 'center', gap: 10 , flexWrap: "wrap"}}>
            ...
            {showModalGerenciar && (
                <div className="modal-overlay" style={{ zIndex: 1100, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' , flexWrap: "wrap"}}>
                    <div style={{ background: '#1a202c', padding: 30, borderRadius: 8, width: '100%', maxWidth: '1100px', display: 'flex', gap: 20, border: '1px solid #4a5568', maxHeight: '90vh', overflowY: 'auto' , flexWrap: "wrap"}}>
                        ...
                        <div style={{ flex: 1, minWidth: '300px' }}>
                            ...
                            <form onSubmit={handleSaveCargo} style={{ display: 'flex', flexDirection: 'column', gap: 15, marginBottom: 20, background: '#2d3748', padding: 15, borderRadius: 8, border: '1px solid #4a5568' }}>
                                ...
                                <button type="submit" style={{ background: '#3182ce', color: 'white', border: 'none', padding: '10px', borderRadius: 4, cursor: 'pointer', fontWeight: 'bold' , whiteSpace: "nowrap", flexShrink: 0}}>Adicionar Cargo</button>
                            </form>
                            ...
                            <div style={{ maxHeight: 300, overflowY: 'auto', border: '1px solid #4a5568', borderRadius: 5, background: '#2d3748', boxSizing: 'border-box' }}>
                                ...
                                {configCargos.map(cfg => (
                                    <div key={cfg.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', padding: '12px 15px', borderBottom: '1px solid #4a5568', boxSizing: 'border-box', color: 'white', fontSize: '0.9rem' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column' , flexWrap: "wrap"}}>
                                            <strong style={{ textTransform: 'capitalize' }}>{cfg.cargo_alvo}</strong>
                                            <span style={{ fontSize: '0.75rem', color: cfg.exige_foto ? '#8B5CF6' : '#e53e3e' }}>
                                        ...
                                        <div style={{ textAlign: 'right', minWidth: '30px', marginLeft: 10, display: 'flex', justifyContent: 'flex-end', alignItems: 'center' , flexWrap: "wrap"}}>
                        ...
                        <div style={{ flex: 1.5, borderLeft: '1px solid #4a5568', paddingLeft: 20, minWidth: '400px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: 20 }}>
                                <h3 style={{ margin: 0, color: '#8B5CF6' }}>Gerenciar Itens do Checklist</h3>
                            ...
                            <form onSubmit={handleSaveItem} style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 100px 100px', gap: 10, marginBottom: 20, background: '#2d3748', padding: 15, borderRadius: 8, border: '1px solid #4a5568', alignItems: 'flex-end', boxSizing: 'border-box' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column' , flexWrap: "wrap"}}>
                                    ...
                                    >
                                        ...
                                        <option value="ADD_NEW" style={{ fontWeight: 'bold', color: '#8B5CF6' }}>+ Adicionar categoria...</option>
                                    </select>
                                </div>
                                ...
                                <button type="submit" style={{ width: '100%', height: '40px', background: '#3182ce', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 5, fontWeight: 'bold' , whiteSpace: "nowrap", flexShrink: 0}}>
                            ...
                            <div style={{ maxHeight: 350, overflowY: 'auto', background: '#2d3748', borderRadius: 5, border: '1px solid #4a5568' }}>
                                <table style={{ width: '100%', color: 'white', textAlign: 'left', fontSize: '0.9rem', borderCollapse: 'collapse' }}>
                                    <thead style={{ background: '#1a202c' }}>
                                        <tr>
                                            <th style={{ padding: '10px 15px', color: '#8B5CF6' }}>ITEM</th>
                                            <th style={{ padding: '10px 15px', color: '#8B5CF6' }}>CATEGORIA</th>
                                            <th style={{ padding: '10px 15px', color: '#8B5CF6', textAlign: 'center' }}>QTD</th>
                                            <th style={{ padding: '10px 15px', color: '#8B5CF6', textAlign: 'right' }}>AÇÕES</th>
                                        </tr>
                                    </thead>
                                    <tbody>

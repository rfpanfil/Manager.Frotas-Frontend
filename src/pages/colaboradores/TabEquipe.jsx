...
export default function TabEquipe() {
    ...
    const { data: bases = [] } = useQuery({ queryKey: ['bases'], queryFn: async () => (await api.get('/bases/')).data , staleTime: 300000 });
    const { data: cargos = [] } = useQuery({ queryKey: ['cargos'], queryFn: async () => (await api.get('/usuarios/cargos/lista')).data , staleTime: 300000 });
    const { data: responseColab } = useQuery({ queryKey: ['colaboradores'], queryFn: async () => await api.get('/colaboradores/') , staleTime: 300000 });
    
    const colaboradores = (responseColab?.data || []).sort((a, b) => {
    ...
    const [precisaAcesso, setPrecisaAcesso] = useState(false);
    ...
    // Removido useEffect que invalidava as queries na montagem
    ...
    function RenderStatusCNH({ data }) {
        ...
        if (status === 'Vencida') {
            ...
            cor = '#8B5CF6'; icone = <CheckCircle size={14} />;
        }
    ...
    async function handleDelete(id) {
        if (confirm("Tem certeza que deseja excluir este colaborador?")) {
            try { await api.delete(`/colaboradores/${id}`); queryClient.invalidateQueries({ queryKey: ['colaboradores'] }); }
        ...
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    {!isReadOnly && can("colaboradores.criar") && (
                        <button onClick={() => abrirModal()} className="btn-add" style={{ backgroundColor: '#8B5CF6', color: '#fff', height: '38px' }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                backgroundColor: filtroStatusCNH === status ?
                                    (status === 'Vencida' ? '#e53e3e' : status === 'Prestes a vencer' ? '#f6ad55' : '#8B5CF6')
                                    : '#2d3748',
                ...
                <table>
                    ...
                    <tbody>
                        {colaboradoresFiltrados.map((m) => {
                            ...
                            const statusColor = m.status === 'Inativo' ? '#e53e3e' : '#8B5CF6';
                                    ...
                                    <td style={{ textAlign: 'center' }}>
                                        ...
                                        {m.cnh_path ? (
                                            <a href={`${api.defaults.baseURL}/colaboradores/${m.id}/cnh`} target="_blank" rel="noopener noreferrer"
                                                style={{ fontWeight: 'bold', color: '#fff', backgroundColor: '#8B5CF6', padding: '4px 10px', borderRadius: '15px', textDecoration: 'none', fontSize: '0.8rem' }}
                                            >
                                                {m.tipo_cnh}
                                            </a>
                                        ) : (
                                    ...
                                    {!isReadOnly && (
                                        <td style={{ textAlign: 'right' }}>
                                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                                                {can("colaboradores.criar") && <button onClick={() => abrirModal(m)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3182ce' }} title="Editar"><Edit size={18} /></button>}
            ...
            {modalAberto && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div className="form-card" style={{ position: 'relative', width: '600px', maxHeight: '90vh', overflowY: 'auto', background: '#1a202c', border: '1px solid #4a5568', padding: '30px' }}>
                        ...
                        <h2 style={{ marginTop: 0, color: '#8B5CF6', marginBottom: '20px' }}>{editandoId ? 'Editar Colaborador' : 'Novo Colaborador'}</h2>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                ...
                                <div className="input-group" style={{ marginTop: '0px', border: '1px dashed #4a5568', padding: '10px', borderRadius: '5px' }}>
                                    <label htmlFor="upload-cnh" style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', color: '#a0aec0', fontSize: '0.85rem' }}>
                                        <UploadCloud size={18} />
                                        {arquivoCNH ? arquivoCNH.name : (form.cnh_path ? "Substituir Doc..." : "Anexar CNH/Doc")}
                                    </label>
                                    <input id="upload-cnh" type="file" accept="image/*,application/pdf" onChange={handleUploadCNH} style={{ display: 'none' }} />
                                </div>
                            </div>

                                                        <div style={{ marginTop: 15, background: '#2d3748', padding: 15, borderRadius: 8 }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', color: 'white' }}>
                                    <input type="checkbox" checked={precisaAcesso} onChange={e => setPrecisaAcesso(e.target.checked)} />
                                    Este colaborador precisará de acesso ao sistema (Login)?
                                </label>
                                {precisaAcesso && (
                                    <div style={{ marginTop: 10, color: '#ecc94b', fontSize: '0.85rem', padding: 10, borderLeft: '3px solid #ecc94b', background: 'rgba(236, 201, 75, 0.1)' }}>
                                        Para criar um colaborador com acesso, vá até <strong>Administração {'>'} Usuários</strong> e clique em 'Novo Usuário'.
                                    </div>
                                )}
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
                                ...
                                <button type="submit" className="btn-add" style={{ backgroundColor: '#8B5CF6', color: '#fff' }}>Salvar</button>
                            </div>
                        </form>
                    </div>
                </div>

...
export default function Auditoria() {
    ...
    // Filtros e Opções Dropdown
    const [opcoes, setOpcoes] = useState({ usuarios: [], tabelas: [], acoes: [] });
    ...
    return (
        <div>
            <div className="header-wrapper" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
                <h1><ShieldAlert style={{ marginRight: '10px', color: '#f6ad55' }} /> Auditoria (Log de Atividades)</h1>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button onClick={exportarExcel} className="btn-add" style={{ backgroundColor: '#3182ce', color: 'white', height: '40px' }} title="Baixar Planilha">
            ...
            <div className="table-container" style={{ width: '100%', overflowX: 'auto' }}>
                {carregando ? (
                    <div style={{ textAlign: 'center', padding: '50px', color: '#00d68f' }}> <Activity size={32} className="spin" style={{ animation: 'spin 1s linear infinite' }} /> <p>Buscando no banco...</p> </div>
                ) : (
                        ...
                        <tbody>
                            {logs.map((l) => (
                                <tr key={l.id}>
                                    ...
                                    <td>
                                        <span style={{
                                            background: l.acao === 'CRIAR' ? 'rgba(0, 214, 143, 0.1)' : l.acao === 'EXCLUIR' ? 'rgba(229, 62, 62, 0.1)' : 'rgba(246, 173, 85, 0.1)',
                                            color: l.acao === 'CRIAR' ? '#00d68f' : l.acao === 'EXCLUIR' ? '#e53e3e' : '#f6ad55',

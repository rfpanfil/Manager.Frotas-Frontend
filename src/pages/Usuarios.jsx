...
export default function Usuarios() {
    ...
    const can = useCan();
    const [abaAtiva, setAbaAtiva] = usePersistedTab('geral', 'usuarios');
  const queryClient = useQueryClient();
  const { data: usuarios = [], isLoading: loadingUsuarios } = useQuery({ queryKey: ['usuarios'], queryFn: async () => (await api.get('/usuarios/')).data });
  const { data: cargos = [], isLoading: loadingCargos } = useQuery({ queryKey: ['cargos'], queryFn: async () => (await api.get('/usuarios/cargos/lista')).data });
  const { data: bases = [], isLoading: loadingBases } = useQuery({ queryKey: ['bases'], queryFn: async () => (await api.get('/bases/')).data });
  const loading = loadingUsuarios || loadingCargos || loadingBases;
  ...
  useEffect(() => {
    if (!formData.cargo && cargos.length > 0) {
  ...
  async function handleEdit(usuario) {
    ...
    try {
      ...
      toast.dismiss(tid);
      toast.error("Erro ao carregar dados do usurio.");
    }
  }
  ...
  async function handleDelete(id) {
    ...
    try { await api.delete(`/usuarios/${id}`); queryClient.invalidateQueries({ queryKey: ['usuarios'] }); }
  ...
  return (
    <div>
      ...
      <div className="form-card" style={{ borderLeft: formData.id ? '4px solid #3182ce' : '4px solid #8B5CF6' }}>
        <h3 style={{ marginTop: 0, color: formData.id ? '#3182ce' : '#8B5CF6' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexWrap: 'wrap', gap: '15px' }}>
          ...
          {isOperacional && (
            <div style={{ width: '100%', background: 'rgba(139, 92, 246, 0.05)', padding: '15px', borderRadius: '5px', border: '1px dashed #8B5CF6', marginTop: '10px' }}>
              <div style={{ color: '#8B5CF6', fontWeight: 'bold', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <div style={{ width: '100%', display: 'flex', gap: '10px', marginTop: '10px' }}>
            <button type="submit" className="btn-add" style={{ backgroundColor: formData.id ? '#3182ce' : '#8B5CF6' }}>
                ...
                <table style={{ margin: 0 }}>
                  ...
                  <tbody>
                    {usuariosAgrupados[cargo].map(u => (
                      <tr key={u.id}>
                        <td>#{u.id}</td>
                        <td>{u.nome} {u.id === currentUser?.id && <span style={{ color: '#8B5CF6' }}>(Você)</span>}</td>
                        <td>{u.login}</td>
                        <td>{u.celular}</td>

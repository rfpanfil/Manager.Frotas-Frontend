...
const customStyles = {
    ...
    option: (base, state) => ({
        ...base,
        backgroundColor: state.isFocused ? '#8B5CF6' : '#1a202c',
...
export default function MapaFrota() {
    const [abaAtiva, setAbaAtiva] = usePersistedTab('geral', 'mapafrota');
    const { user, can } = useAuth(); // <--- HOOK INICIADO
    const queryClient = useQueryClient();
    const { data: ativos = [], isLoading: loading, refetch: carregarFrota } = useQuery({
        queryKey: ['frotaAtiva'],
        queryFn: async () => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
            ...
            <div style={{ padding: '15px 20px', background: '#1a202c', borderBottom: '1px solid #2d3748', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <h2 style={{ color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <Truck size={24} color="#8B5CF6" /> Rotas Ativas (Equipe)
                </h2>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <button
                        ...
                        style={{ background: '#2d3748', border: 'none', color: loading ? '#8B5CF6' : 'white', padding: '10px', borderRadius: '5px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                    >
            ...
            <div style={{ flex: 1, position: 'relative', background: '#e2e8f0' }}>
                                        ...
                                        <div style={{ textAlign: 'center' }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', fontSize: '0.8rem' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                                                    {item.status_conexao === 'Online' ? <Wifi size={14} color="#8B5CF6" /> : <WifiOff size={14} color="#e53e3e" />}
                                                    <span style={{ color: item.status_conexao === 'Online' ? '#8B5CF6' : '#e53e3e', fontWeight: 'bold' }}>
                                                        {item.status_conexao}
                                                    </span>
                                                </div>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px' }}>
                        <div style={{ width: 10, height: 10, background: '#8B5CF6', borderRadius: '50%' }}></div> Em Rota (Online)
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>

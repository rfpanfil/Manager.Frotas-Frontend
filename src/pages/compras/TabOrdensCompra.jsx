    ...
    async function carregarDados() {
        try {
            const [resOC, resF, resE, resTG, resB, resV, resU, resSP, resCat, resUnid] = await Promise.all([
                api.get('/compras/oc'), api.get('/compras/fornecedores'), api.get('/estoque/itens/resumo'),
                api.get('/opcoes/tipos-gasto'), api.get('/bases'), api.get('/veiculos'),
                api.get('/usuarios'), api.get('/compras/servicos'), api.get('/estoque/categorias').catch(() => ({ data: [] })),
    ...
    return (
        <div style={{ paddingTop: 10 }}>
            ...
            <TabelaOrdensCompra
                ...
                imprimirOC={(oc) => imprimirOC(oc, getNomeEstoque)}
                can={can}

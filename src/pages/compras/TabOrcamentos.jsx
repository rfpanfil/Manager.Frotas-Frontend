...
export default function TabOrcamentos({ scPreSelecionada, clearScPreSelecionada, onAprovar }) {
    ...
    const { data: todasScs = [] } = useQuery({ queryKey: ['scs'], queryFn: async () => (await api.get('/compras/sc')).data });
    const { data: orcamentos = [] } = useQuery({ queryKey: ['orcamentos'], queryFn: async () => (await api.get('/compras/orcamentos')).data });
    const { data: fornecedoresBrutos = [] } = useQuery({ queryKey: ['fornecedores'], queryFn: async () => (await api.get('/compras/fornecedores')).data });
    ...
    const fornecedores = fornecedoresBrutos.filter(f => f.status === 'Ativo');
    const scsPendentes = todasScs.filter(s => s.status === 'Em Análise' || s.status === 'Orçamento Reprovado');
    const scIdsComOrc = [...new Set(orcamentos.map(o => o.solicitacao_id))];
    const scsComOrcamentos = todasScs.filter(s => scIdsComOrc.includes(s.id));
    ...
    async function salvarOrcamentos(e) {
        ...
        try {
            ...
            toast.success("Orçamentos salvos com sucesso!"); setModalAberto(false); queryClient.invalidateQueries({ queryKey: ['orcamentos'] }); queryClient.invalidateQueries({ queryKey: ['scs'] });
    ...
    async function excluirOrcamentoUnico(orcId, index) {
        ...
        if (orcId) {
            try {
                ...
                setFormOrcamentos(novos); queryClient.invalidateQueries({ queryKey: ['orcamentos'] }); queryClient.invalidateQueries({ queryKey: ['scs'] });
    ...
    return (
        <div style={{ paddingTop: 10 }}>
            ...
            <TabelaOrcamentos
                ...
                baixarComparativoPDF={(sc) => baixarComparativoPDF(sc, orcamentos, bases, getNomeItem, user)}

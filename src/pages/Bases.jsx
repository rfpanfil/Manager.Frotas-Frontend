...
import { Building, Plus, Trash2, Search, Pencil } from 'lucide-react';
import toast from 'react-hot-toast';

import ModalEditarBase from '../components/bases/ModalEditarBase';

export default function Bases() {
    const [abaAtiva, setAbaAtiva] = usePersistedTab('geral', 'bases');
    const { can } = useAuth();
    ...
    const [baseEditando, setBaseEditando] = useState(null);
    ...
    async function handleDelete(id) {
        ...
        try { await api.delete(`/bases/${id}`); queryClient.invalidateQueries({ queryKey: ['bases'] }); } catch(e) { toast.error("Erro ao excluir."); }
    }
        ...
        <div style={{padding: '20px', width: '100%', margin: '0 auto'}}>
            ...
            {can('bases.gerenciar') && (
                <div style={{display: 'flex', gap: '10px', marginBottom: '20px'}}>
                    ...
                    />
                    <button onClick={handleAdd} className="btn-add" style={{background: '#8B5CF6', color: '#fff'}}>
                            ...
                            <tr key={b.id}>
                                ...
                                {can('bases.gerenciar') && (
                                    <td style={{textAlign: 'center'}}>
                                        <button onClick={() => { setBaseEditando(b); setModalEditarAberto(true); }} style={{color: '#63b3ed', background:'none', border:'none', cursor:'pointer', marginRight: '10px'}} title="Editar">
                                            <Pencil size={18}/>
                                        </button>
                                        {can("bases.gerenciar") && ( <button onClick={() => handleDelete(b.id)} style={{color: '#e53e3e', background:'none', border:'none', cursor:'pointer'}} title="Excluir">
                                            <Trash2 size={18}/>
                                        </button> )}
                                    </td>
                                )}

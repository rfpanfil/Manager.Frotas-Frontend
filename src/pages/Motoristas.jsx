...
export default function Motoristas() {
    const [abaAtiva, setAbaAtiva] = usePersistedTab('geral', 'motoristas');
  ...
  async function handleDelete(id) {
    if (confirm("Tem certeza que deseja excluir este motorista?")) {
      try { await api.delete(`/motoristas/${id}`); queryClient.invalidateQueries({ queryKey: ['motoristas'] }); } 
      ...
      {modalAberto && (
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000}}>
            <div className="form-card" style={{position: 'relative', width: '600px', maxHeight: '90vh', overflowY: 'auto', background: '#1a202c', border: '1px solid #4a5568', padding: '30px'}}>
               ...
               <h2 style={{marginTop: 0, color: '#8B5CF6', marginBottom: '20px'}}>{editandoId ? 'Editar Motorista' : 'Novo Motorista'}</h2>

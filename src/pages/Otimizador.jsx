...
export default function Otimizador() {
    const [abaAtiva, setAbaAtiva] = usePersistedTab('geral', 'otimizador');
    const { user, can } = useAuth();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(false);
    const [resultado, setResultado] = useState(null);
    const { data: veiculos = [] } = useQuery({ queryKey: ['veiculosResumo'], queryFn: async () => (await api.get('/veiculos/')).data });
    ...
    return (
        <div>
            <h1 style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>Otimizador de Rotas</h1>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {linksMapa.map((link, idx) => (
                                <a
                                    ...
                                    rel="noreferrer"
                                    style={{
                                        display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '15px', justifyContent: 'space-between',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '30px', marginTop: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div className="form-card" style={{ borderLeft: '4px solid #3182ce' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '20px', paddingBottom: '15px', borderBottom: '1px solid #333' }}>
                    ...
                    <button onClick={handleOtimizar} disabled={loading} className="btn-add" style={{ width: '100%', height: '50px', fontSize: '1.1rem', backgroundColor: '#8B5CF6', color: '#fff' }}>
                    ...
                    {resultado ? (
                        <div className="card" style={{ border: '1px solid #444' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '20px' }}>
                                <h2 style={{ color: '#8B5CF6', margin: 0 }}>Rota Otimizada</h2>

                                {/* BOTÕES DE AÇÃO: MAPA E PDF */}
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    {can('relatorios.baixar') && ( <button onClick={gerarRelatorioPDF} className="btn-add" style={{ backgroundColor: '#dd6b20', color: '#fff', padding: '8px 15px', fontWeight: 'bold' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '20px', textAlign: 'center' }}>
                                ...
                                <div style={{ background: '#2d3748', padding: '10px', borderRadius: '5px' }}>
                                    ...
                                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#8B5CF6' }}>R$ {resultado.resumo.valor_total}</div>
                                </div>
                                <div style={{ background: '#2d3748', padding: '10px', borderRadius: '5px' }}>
                                ...
                                {resultado.rota_ordenada.map((item, idx) => (
                                    <div key={idx} style={{ marginBottom: '30px', position: 'relative' }}>
                                        <div style={{
                                            ...
                                            background: '#1a202c', border: item.tipo === 'final' ? '2px solid #e53e3e' : '2px solid #8B5CF6',
                                            borderRadius: '50%', width: '28px', height: '28px',
                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            fontSize: '0.8rem', color: item.tipo === 'final' ? '#e53e3e' : '#8B5CF6', fontWeight: 'bold', zIndex: 2
                                        }}>
                                            {item.tipo === 'final' ? <Flag size={14} /> : idx + 1}
                                        </div>

                                        <div style={{ background: '#2d3748', padding: '12px', borderRadius: '8px', borderLeft: item.tipo === 'final' ? '4px solid #e53e3e' : '4px solid #8B5CF6' }}>
                                                ...
                                                {editandoIndex === idx ? (
                                                    <div style={{ display: 'flex', gap: '5px', width: '100%' }}>
                                                        <input
                                                            ...
                                                            style={{ flex: 1, borderRadius: '4px', border: '1px solid #8B5CF6', padding: '4px', background: '#1a202c', color: 'white' }}
                                                        />
                                                        <button onClick={() => salvarEdicao(idx)} style={{ background: '#8B5CF6', border: 'none', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '0 5px' }}>
                                            ...
                                            {item.valor > 0 && <div style={{ color: '#8B5CF6', fontWeight: 'bold', marginTop: '5px' }}>Receber: R$ {item.valor}</div>}
                            ...
                            <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #444' }}>
                                ...
                                {can('rotas.criar') && (
                                    <button
                                        ...
                                        className="btn-add"
                                        style={{ width: '100%', marginTop: '15px', backgroundColor: '#8B5CF6', color: '#fff' }}
                                    >

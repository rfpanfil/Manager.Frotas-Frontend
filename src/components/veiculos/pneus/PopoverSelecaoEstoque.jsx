        ...
        <div className="modal-overlay" style={{ zIndex: 2100 }}>
            <div className="modal-content" style={{ width: '450px', background: '#1a202c' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 15 }}>
                    <h3>Montar em {posicaoAlvo} <br /><span style={{ fontSize: '0.8rem', color: '#8B5CF6' }}>{veiculoMontagem?.placa}</span></h3>
                ...
                <div style={{ maxHeight: 250, overflowY: 'auto', marginBottom: 15 }}>
                    {pneus.filter(p => p.status === 'ESTOQUE' && ((p.dot || '').includes(buscaPopover) || (p.fogo || '').includes(buscaPopover) || (p.medida || '').includes(buscaPopover))).map(p => (
                        <div key={p.id} onClick={() => handleMontarEstoque(p.id)} style={{ padding: 15, background: '#2d3748', marginBottom: 5, borderRadius: 5, cursor: 'pointer', border: '1px solid #4a5568', transition: '0.2s' }} onMouseOver={e => e.currentTarget.style.borderColor = '#8B5CF6'} onMouseOut={e => e.currentTarget.style.borderColor = '#4a5568'}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <strong style={{ color: '#8B5CF6', fontSize: '1.1rem' }}>{p.dot || p.fogo}</strong>

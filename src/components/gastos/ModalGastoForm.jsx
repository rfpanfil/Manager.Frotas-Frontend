...
const customSelectStyles = {
    control: (base, state) => ({
        ...
        borderColor: state.isFocused ? '#8B5CF6' : '#444',
        color: 'white',
        minHeight: '42px',
...
export default function ModalGastoForm({
    ...
    return (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 99999, padding: '10px' }}>
            <div className="modal-card" style={{
                ...
                <form onSubmit={handleSubmit} style={{
                    ...
                    <div style={{ background: 'rgba(139, 92, 246, 0.1)', border: '1px dashed #8B5CF6', padding: '10px', borderRadius: '5px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '15px', justifyContent: 'space-between' }}>
                        <span style={{ color: '#8B5CF6', fontSize: '0.9rem' }}>Tem Nota Fiscal (NFC-e)?</span>
                        <button
                            type="button"
                            onClick={() => setLendoQR(true)}
                            style={{ background: '#8B5CF6', color: '#fff', border: 'none', padding: '8px 15px', borderRadius: '5px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 'bold' }}
                        >
                    ...
                    {/* --- SOLICITANTE / COLABORADOR (SEMPRE VISÍVEL) --- */}
                    <div className="input-group">
                    ...
                    {/* --- SELEÇÃO DE VEÍCULO OU CENTRO DE CUSTO --- */}
                    {[
                    ...
                    {linkQrProtegido && (
                        <div style={{ marginBottom: '15px', padding: '10px', background: 'rgba(139, 92, 246, 0.1)', borderRadius: '5px', border: '1px solid #8B5CF6' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '5px' }}>
                                <label style={{ color: '#8B5CF6', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.85rem' }}>
                            ...
                            <ul style={{ listStyle: 'none', padding: 0, margin: '5px 0' }}>
                                {anexosExistentes.map((nome, i) => (
                                    <li key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #444' }}>
                                        ...
                                        {can("gastos.excluir") && ( <button type="button" onClick={() => handleDeleteAnexo(nome)} style={{ background: 'none', border: 'none', color: '#e53e3e', cursor: 'pointer' }}><Trash2 size={16} /></button> )}
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div className="input-group" style={{ border: '1px dashed #4a5568', padding: '10px', borderRadius: '5px' }}>
                        ...
                        />
                        {arquivos.length > 0 && <small style={{ color: '#8B5CF6' }}>{arquivos.length} novo(s) arquivo(s) selecionado(s)</small>}
                    </div>

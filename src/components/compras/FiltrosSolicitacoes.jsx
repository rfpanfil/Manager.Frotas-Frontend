        ...
        <div className="filter-bar" style={{ background: '#2d3748', padding: 15, borderRadius: 8, marginBottom: 20 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 15, alignItems: 'center', marginBottom: 15 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: '#1a202c', padding: '5px 10px', borderRadius: 5, border: '1px solid #4a5568' }}>
                    ...
                    <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Filtrar por Nº SC, Item..." style={{ background: 'transparent', border: 'none', color: 'white', outline: 'none', flex: 1, minWidth: '150px' }} />
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {can('compras.sc.baixar') && (
                        <button onClick={baixarListaPDF} className="btn-add" style={{  background: 'transparent', border: '1px solid #63b3ed', color: '#63b3ed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                            <FileDown size={16} /> Baixar PDF
                        </button>
                    )}
                    {can('compras.sc.criar') && (
                        <button onClick={abrirModalNovaSC} className="btn-add" style={{  background: '#8B5CF6', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>

        ...
        <div className="modal-overlay" onClick={() => setFornSelecionado(null)}>
            <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '700px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #444', paddingBottom: 10, marginBottom: 15 }}>
                    <h3>Ficha do Fornecedor: <span style={{ color: '#8B5CF6' }}>{fornSelecionado.razao_social}</span></h3>
                ...
                <div style={{ background: '#1a202c', padding: 15, borderRadius: 8, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
                    ...
                    <div><small style={{ color: '#a0aec0' }}>Status:</small><div style={{ color: fornSelecionado.status === 'Ativo' ? '#8B5CF6' : '#e53e3e' }}>{fornSelecionado.status}</div></div>
                            ...
                            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                                {ocs.filter(oc => oc.fornecedor_id === fornSelecionado.id).map(oc => (
                                    <li key={oc.id} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed #444', padding: '5px 0' }}>
                                        ...
                                        <span style={{ fontSize: '0.8rem', color: oc.status === 'Cancelada' ? '#e53e3e' : '#8B5CF6' }}>{oc.status}</span>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                    {can('compras.baixar') && ( <button onClick={() => baixarResumoPDF(fornSelecionado)} style={{ flex: 1, background: 'transparent', border: '1px solid #63b3ed', color: '#63b3ed', padding: 10, borderRadius: 5, cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 5 }}>

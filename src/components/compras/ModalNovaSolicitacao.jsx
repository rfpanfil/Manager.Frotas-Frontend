...
const customSelectStyles = {
    control: (base, state) => ({
        ...
        boxShadow: state.isFocused ? '0 0 0 1px #8B5CF6' : 'none', '&:hover': { borderColor: '#8B5CF6' }
    }),
            ...
            <div className="modal-content" style={{ background: '#1a202c', width: '100%', maxWidth: '1000px', maxHeight: '95vh', overflowY: 'auto', padding: 30, borderRadius: 8 }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    <div style={{ background: '#2d3748', padding: 20, borderRadius: 8, border: '1px solid #4a5568' }}>
                        <h3 style={{ marginTop: 0, color: '#8B5CF6', marginBottom: 15, display: 'flex', alignItems: 'center', gap: 10 }}><FileText size={20} /> 1. Informações Gerais do Pedido</h3>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 15 }}>
                            <div>
                                ...
                                <select value={formCapa.tipo_gasto || ''} onChange={e => setFormCapa({ ...formCapa, tipo_gasto: e.target.value })} style={inputStyle}>
                                    <option value="">Selecione...</option>
                                    {tiposGasto.map(t => <option key={t.id} value={t.nome}>{t.nome}</option>)}
                                </select>
                            </div>
                            <div style={{ gridColumn: '1 / -1' }}><label style={{ display: 'block', marginBottom: 5 }}>Justificativa / Observação Geral</label><textarea rows="2" value={formCapa.observacoes} onChange={e => setFormCapa({ ...formCapa, observacoes: e.target.value })} style={inputStyle} /></div>
                        </div>
                    </div>
                                ...
                                {itemAtual.classificacao === 'PRODUTO' ? (
                                    <>
                                        <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                                            ...
                                            <span onClick={() => setModalNovoItemEstoque(true)} style={{ color: '#8B5CF6', cursor: 'pointer', fontSize: '0.85rem' }}>+ Cadastrar Novo Modelo no Estoque</span>
                                        </label>
                                        <Select
                                        ...
                                        {carrinhoItens.map((item, idx) => (
                                            <tr key={idx} style={{ borderBottom: '1px solid #2d3748' }}>
                                                ...
                                                <td style={{ padding: 10 }}>
                                                    ...
                                                    <span style={{ marginLeft: 10, fontSize: '0.7rem', padding: '2px 5px', background: item.classificacao === 'PRODUTO' ? '#8B5CF620' : '#ecc94b20', color: item.classificacao === 'PRODUTO' ? '#8B5CF6' : '#ecc94b', borderRadius: 4 }}>{item.classificacao}</span>
                                                </td>
                                                <td style={{ padding: 10, textAlign: 'center', fontWeight: 'bold' }}>{item.quantidade}</td>
                                                <td style={{ padding: 10, textAlign: 'center' }}>
                                                    {can("compras.excluir") && ( <button type="button" onClick={() => removerItemDoCarrinho(idx)} style={{ background: 'none', border: 'none', color: '#e53e3e', cursor: 'pointer' }}><Trash2 size={16} /></button> )}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                    ...
                    <button type="submit" className="btn-add" style={{ width: '100%', padding: 15, fontSize: '1.1rem', background: '#8B5CF6', color: '#fff', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
